...
class CommandProcessingService {
  ...
  private async processOfflineCommand(command: string): Promise<CommandResult> {
    try {
      ...
      const userDisplayName = userCodename || (userName ? `${userRank} ${userName}` : userRank);

      // Use the offline command processor
      const result = await processOfflineCommand(command, userDisplayName);
  ...
  private async processStandardCommand(command: string, alternatives: string[] = []): Promise<CommandResult> {
    try {
      ...
      if (typeof result === 'string') {
        ...
        // Add user's preferred name to the response for personalization if appropriate
        if (success && response && !response.includes(userDisplayName) && Math.random() > 0.5) {
          const personalizedPrefixes = [
            `${userDisplayName}, `,
            `Got it ${userDisplayName}. `,
            `Yes ${userDisplayName}, `
          ];
